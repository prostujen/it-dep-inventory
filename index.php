...
$sql = "SELECT d.*, n.ip_address, n.subnet_mask, n.gateway, n.dns_server, n.mac_address 
        FROM devices d 
...
<?php if (isset($_GET['success'])): ?>
    <div class="alert alert-success p-3 mb-4 no-print">
        ...
        <?php 
            if ($_GET['success'] === 'created') echo 'Пристрій успішно додано!';
            elseif ($_GET['success'] === 'updated') echo 'Дані пристрою успішно оновлено!';
            elseif ($_GET['success'] === 'deleted') echo 'Пристрій успішно видалено!';
        ?>
    </div>
<?php endif; ?>
...
<?php if (isset($_GET['error'])): ?>
    <div class="alert alert-danger p-3 mb-4 no-print">
        ...
        <?php 
            if ($_GET['error'] === 'empty_fields') echo 'Будь ласка, заповніть усі обов\'язкові поля.';
            elseif ($_GET['error'] === 'not_found') echo 'Пристрій не знайдено.';
            elseif ($_GET['error'] === 'access_denied') echo 'Доступ заборонено! Тільки адміністратори мають право виконувати цю операцію.';
            elseif ($_GET['error'] === 'duplicate_numbers') echo 'Пристрій з таким інвентарним або серійним номером вже існує.';
            elseif ($_GET['error'] === 'invalid_ip') echo 'Некоректна IP-адреса.';
            elseif ($_GET['error'] === 'invalid_subnet') echo 'Некоректна маска підмережі.';
            elseif ($_GET['error'] === 'invalid_gateway') echo 'Некоректний шлюз.';
            elseif ($_GET['error'] === 'invalid_dns') echo 'Некоректний DNS-сервер.';
            elseif ($_GET['error'] === 'ip_conflict') {
                $conflicting_inv = isset($_GET['conflicting_inv']) ? htmlspecialchars($_GET['conflicting_inv']) : 'іншим пристроєм';
                echo 'Конфлікт IP-адреси! Вказана адреса вже використовується активним обладнанням з інвентарним номером: <strong>' . $conflicting_inv . '</strong>.';
            } elseif ($_GET['error'] === 'mac_conflict') {
                $conflicting_inv = isset($_GET['conflicting_inv']) ? htmlspecialchars($_GET['conflicting_inv']) : 'іншим пристроєм';
                echo 'Конфлікт MAC-адреси! Вказана фізична адреса вже використовується активним обладнанням з інвентарним номером: <strong>' . $conflicting_inv . '</strong>.';
            } elseif ($_GET['error'] === 'db_error') {
                echo 'Помилка бази даних: ' . htmlspecialchars($_GET['msg'] ?? '');
            } else {
                echo 'Виникла невідома помилка.';
            }
        ?>
    </div>
<?php endif; ?>
    ...
    <form method="GET" action="index.php" class="row g-3">
        <div class="col-md-3">
            ...
            <div class="input-group">
                ...
                <input type="text" name="q" class="form-control form-control-custom filter-row-input" placeholder="Назва, модель, S/N..." value="<?php echo htmlspecialchars($search); ?>">
            </div>
        </div>
        
        <div class="col-md-2">
            <label class="form-label text-muted small">Аудиторія / Локація</label>
            <select name="location" class="form-select form-select-custom filter-row-input">
        ...
        <div class="col-md-2">
            ...
            <select name="type" class="form-select form-select-custom filter-row-input">
        ...
        <div class="col-md-2">
            ...
            <select name="status" class="form-select form-select-custom filter-row-input">
                <option value="">Всі статуси</option>
                <option value="в роботі" <?php echo ($filter_status === 'в роботі') ? 'selected' : ''; ?>>В роботі</option>
                <option value="очікує перевірки" <?php echo ($filter_status === 'очікує перевірки') ? 'selected' : ''; ?>>Очікує перевірки</option>
        ...
        <div class="col-md-3 d-flex align-items-end gap-2">
            <button type="submit" class="btn btn-custom-primary w-100 btn-filter-apply">
                <i class="bi bi-filter"></i> Застосувати
            </button>
            <a href="index.php" class="btn btn-custom-secondary btn-filter-reset" title="Скинути фільтри">
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($devices as $dev): ?>
                        <tr>
                            <td class="fw-bold text-dark"><?php echo htmlspecialchars($dev['inventory_number']); ?></td>
                            <td>
                                <div class="fw-semibold text-dark"><?php echo htmlspecialchars($dev['name']); ?></div>
                            ...
                            <td>
                                 <?php if ($dev['ip_address']): ?>
                                     ...
                                     <?php if ($dev['mac_address']): ?>
                                         <div class="text-muted small mt-1" style="font-size: 0.75rem;">MAC: <code><?php echo htmlspecialchars($dev['mac_address']); ?></code></div>
                                     <?php endif; ?>
                                 <?php else: ?>
                            ...
                            <td>
                                <?php 
                                    ...
                                    elseif ($dev['status'] === 'очікує перевірки') $statusClass = 'badge-pending';
                                ...
                                <span class="badge-custom <?php echo $statusClass; ?>">
                                    <span class="pulsing-dot <?php 
                                        ...
                                        elseif ($dev['status'] === 'очікує перевірки') echo 'pulsing-dot-info';
                                ...
                                <div class="d-flex gap-2 justify-content-center">
                                    ...
                                    <?php if ($is_admin): ?>
                                        <button class="btn btn-sm btn-custom-secondary edit-device-btn" 
                                                 ...
                                                 data-is-pc="<?php echo $dev['is_pc']; ?>"
                                                 data-bs-toggle="modal" 
                                                 data-bs-target="#editDeviceModal"
                                                 title="Редагувати">
                                            <i class="bi bi-pencil-square text-warning"></i>
                                        </button>
                                        <a href="actions/delete_device.php?id=<?php echo $dev['id']; ?>" class="btn btn-sm btn-custom-secondary delete-device-btn"
                                           title="Видалити">
                                            <i class="bi bi-trash3-fill text-danger"></i>
                                        </a>
                                    <?php elseif ($dev['status'] !== 'на ремонті' && $dev['status'] !== 'очікує перевірки' && $dev['status'] !== 'списано'): ?>
                                        <a href="device_view.php?id=<?php echo $dev['id']; ?>&open_ticket=1" class="btn btn-sm btn-warning text-dark fw-semibold" title="Подати заявку">
                                            <i class="bi bi-exclamation-triangle-fill"></i> Заявка
                                        </a>
                                    <?php endif; ?>
                                </div>
                            </td>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="status" class="form-select form-select-custom" required>
                                ...
                                <option value="очікує перевірки">Очікує перевірки</option>
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="is_pc" class="form-select form-select-custom" required>
                                ...
                                <option value="1">Так (ПК / Ноутбук / Сервер)</option>
                                <option value="0">Ні (інше обладнання)</option>
                            </select>
                        </div>
                        <div class="col-md-12">
                            <label class="form-label text-muted small">Відповідальна особа *</label>
                            <input type="text" name="responsible_person" class="form-control form-control-custom" required>
                        </div>
                        
                        <!-- Необов'язкові поля внизу -->
                        <div class="col-md-6">
                            <label class="form-label text-muted small">Інвентарний номер</label>
                            <input type="text" name="inventory_number" class="form-control form-control-custom">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label text-muted small">Серійний номер</label>
                            <input type="text" name="serial_number" class="form-control form-control-custom">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label text-muted small">Модель</label>
                            <input type="text" name="model" class="form-control form-control-custom">
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="date" name="accepted_date" class="form-control form-control-custom">
                        </div>
                        ...
                        <div class="col-md-12">
                            <label class="form-label text-muted small">MAC-адреса</label>
                            <input type="text" name="mac_address" class="form-control form-control-custom" placeholder="Напр: 00:1A:2B:3C:4D:5E">
                        </div>
                    </div>
                </div>
            ...
            <form action="actions/save_device.php" method="POST">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-12">
                            <label class="form-label text-muted small">Тип пристрою (ПК / Комп'ютер) *</label>
                            <select name="is_pc" id="edit-is-pc" class="form-select form-select-custom" required>
                                <option value="1">Так (ПК / Ноутбук / Сервер)</option>
                                <option value="0">Ні (інше обладнання)</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-6">
                            <label class="form-label text-muted small">Інвентарний номер</label>
                            <input type="text" name="inventory_number" id="edit-inv" class="form-control form-control-custom">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label text-muted small">Серійний номер</label>
                            <input type="text" name="serial_number" id="edit-sn" class="form-control form-control-custom">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label text-muted small">Модель</label>
                            <input type="text" name="model" id="edit-model" class="form-control form-control-custom">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label text-muted small">Поточна локація</label>
                            <input type="text" name="location" id="edit-loc" class="form-control form-control-custom">
                        </div>
                        <div class="col-md-12">
                            <label class="form-label text-muted small">Дата прийняття</label>
                            <input type="date" name="accepted_date" id="edit-date" class="form-control form-control-custom">
                        </div>
                    </div>
                </div>
                <div class="modal-footer modal-footer-custom">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    editButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            document.getElementById('edit-is-pc').value = this.getAttribute('data-is-pc');
        });
    });
});
</script>
